  ...
  module DeviceAuthorizationGrant
    module OAuth
      class DeviceCodeRequestTest < ActiveSupport::TestCase
        ...
        test '#authorize raises ExpiredToken error if the device grant is ' \
          'expired and deletes the device grant' do
          request =
            DeviceCodeRequest.new(@server, @application, @expired_device_grant)
        ...
        test '#authorize raises AuthorizationPending error if the device ' \
          'grant has not been verified' do
          request = DeviceCodeRequest
                    .new(@server, @application, @unverified_device_grant)
        ...
        test '#authorize issues a new token if the device grant has ' \
          'been verified' do
          request = DeviceCodeRequest
                    .new(@server, @application, @verified_device_grant)
        ...
        test '#authorize with a verified device grant issues a new token ' \
          'with same device grant scopes' do
          request = DeviceCodeRequest
                    .new(@server, @application, @verified_device_grant)
          request.authorize
          assert_equal \
            @verified_device_grant.scopes,
            Doorkeeper::AccessToken.last.scopes.to_s
        end

        test '#authorize with a verified device grant deletes the ' \
          'device grant' do
          request = DeviceCodeRequest
                    .new(@server, @application, @verified_device_grant)
          request.authorize
        ...
        test '#authorize updates the polling interval of an unverified ' \
          'device grant' do
          assert_nil @unverified_device_grant.last_polling_at
          request = DeviceCodeRequest
                    .new(@server, @application, @unverified_device_grant)
          suppress(Errors::AuthorizationPending) { request.authorize }
          refute_nil @unverified_device_grant.reload.last_polling_at
        end
        ...
        test 'it requires the device grant application to be the same ' \
          'as the client' do
